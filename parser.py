...
if __name__ == "__main__":
    ...
    try:
        ...
        lookup.update(readDefinitions(path, "flavor_events"))
        lookup.update(readDefinitions(path, "USA_dlc"))
        events = readDefinitions(path, "generic_events")
        events.update(readDefinitions(path, "flavor_events"))
        events.update(readDefinitions(path, "EU4"))
        events.update(readDefinitions(path, "muslim_dlc"))
        events.update(readDefinitions(path, "Purple_Phoenix"))
        events.update(readDefinitions(path, "USA_dlc"))
